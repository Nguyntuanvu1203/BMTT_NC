    ...
    def call_api_encrypt(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                encrypted_message = data.get("encrypted_message")  # Tránh KeyError
                if encrypted_message:
                    ...
                    QMessageBox.warning(self, "API Error", "Key 'encrypted_message' not found in API response!")
            else:
                QMessageBox.warning(self, "API Error", f"Error while calling API: {response.status_code}")
